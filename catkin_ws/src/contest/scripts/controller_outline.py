...
class controller:
    def __init__(self):
        ...
        # self.rx_center_point = 250 -> 2차선 시작_1
        self.lx_center_point = 60
        self.rx_center_point = 250 # -> 2차선 시작_2
        # self.lx_center_point = 70
        # self.rx_center_point = 250 # -> 2차선 시작_3
       
        # self.denominator = 420 -> 2차선 시작_1
        self.denominator = 450 # -> 2차선 시작_2
    ...
    def Avoid_static(self):
        if self.static_count % 2 == True : #왼쪽 턴
            ...
            self.Lidar_data_change(700,1025,0.5)
            ...
            if self.num_objects:
                while self.num_objects :
                    rospy.loginfo(f"{self.num_objects}, check")
                    self.jet_Control(-0.8,145)
                self.jet_Control(0,0)
                self.Lidar_data_change(420,470,0.5)
            
            elif self.num_objects != True:
                right_angle = 700
                left_angle = 1025
                dist = 0.5

                while True:
                    self.Lidar_data_change(right_angle+10, left_angle+10, dist)
                    # right_angle += 10
                    left_angle += 10
                    start_time = rospy.get_time()
                    end_time = rospy.get_time()
            ...
            if self.num_objects == False:
                while self.num_objects != True :
                    rospy.loginfo(f"{self.num_objects}, fuck")
                ...
                while True:
                    ...
                    if self.num_objects == False:
                        while self.num_objects != True :
                            rospy.loginfo(f"{self.num_objects}, check")
                            self.jet_Control(0.35,145)
                        self.jet_Control(0,0)
                        break    

            self.Lidar_data_change(921,1025,0.6)
            self.follow_line = 'L'
            self.Lane_follow_driving(145)

        elif self.static_count % 2 == False : #오른쪽 턴
            rospy.loginfo("right_turn")
            self.Lidar_data_change(879,1311,0.5)
            
            while self.num_objects :
                self.jet_Control(0.9,140)
            self.jet_Control(0,0)
            self.Lidar_data_change(1500,1554,0.5)
            
            while self.num_objects != True :
                self.jet_Control(-0.5,140)
                
            self.Lidar_data_change(879,1067,0.5)
            self.follow_line = 'R'
            self.Lane_follow_driving(145)
